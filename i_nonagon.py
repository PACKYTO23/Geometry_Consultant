...
SYMMETRY_ROTATION = DEGREES / SIDES  # 40°
INTERNAL_ANGLE = SEMICIRCLE - SYMMETRY_ROTATION  # 140°
HALF_ANGLE_EQUAL_TRIANGLES = DEGREES / (2 * SIDES)  # 32.72727272727272...°


def side_length():
    """Returns the results of the 5 properties left given the side length."""
    s_length = float(input(f"Enter the length of the {N}'s side:\n"))
...
def apothem_icr_length():
    ...
    ap_length = float(input(f"Enter the length of the {N}'s apothem or inscribed circle radius:\n"))

    if ap_length < 0:
        print("Negative values are invalid as anti-figures are yet to be discovered.")
        apothem_icr_length()
    else:
        side = 2 * (ap_length / (sin(radians(70)) / sin(radians(20))))
...
def perimeter_length():
    ...
    p_length = float(input(f"Enter the length of the {N}'s perimeter:\n"))
...
def area_surface():
    ...
    a_surface = float(input(f"Enter the surface of the {N}'s area:\n"))
...
def circumradius_length():
    ...
    ccr_length = float(input(f"Enter the length of the {N}'s circumscribed circle radius:\n"))
...
def calculate_nonagon(characteristic):
